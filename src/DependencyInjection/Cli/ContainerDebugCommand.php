...
namespace Swift\DependencyInjection\Cli;
...
class ContainerDebugCommand extends \Swift\Console\Command\AbstractCommand {
    ...
    protected function configure(): void {
        $this
            ...
            ->setDescription( 'Display current services for the application' )
            ->setHelp( <<<'EOF'
...
EOF
    ...
    protected function execute( InputInterface $input, OutputInterface $output ): int {
        ...
        if (Utils::isProductionMode($this->configuration)) {
            $errorIo->error('Debugging the container is not possible in production mode');
        ...
        if ( $input->getOption( 'env-vars' ) ) {
        ...
        } else if ( $envVar = $input->getOption( 'env-var' ) ) {
            $options = [ 'env-vars' => true, 'name' => $envVar ];
        } else if ( $input->getOption( 'types' ) ) {
            $options             = [];
            $options[ 'filter' ] = [ $this, 'filterToServiceTypes' ];
        } else if ( $input->getOption( 'parameters' ) ) {
            $parameters = [];
            ...
            $options = [];
        } else if ( $parameter = $input->getOption( 'parameter' ) ) {
            $options = [ 'parameter' => $parameter ];
        } else if ( $input->getOption( 'tags' ) ) {
            $options = [ 'group_by' => 'tags' ];
        } else if ( $tag = $input->getOption( 'tag' ) ) {
            $options = [ 'tag' => $tag ];
        } else if ( $name = $input->getArgument( 'name' ) ) {
            $name    = $this->findProperServiceName( $input, $errorIo, $object, $name, $input->getOption( 'show-hidden' ) );
            $options = [ 'id' => $name ];
        } else if ( $input->getOption( 'deprecations' ) ) {
        ...
        $options[ 'is_debug' ]       = Utils::isDebug( $this->configuration );
        
        try {
            $helper->describe( $this->io, $object, $options );
    ...
    private function findProperServiceName( InputInterface $input, SymfonyStyle $io, Container $builder, string $name, bool $showHidden ): string {
        $name = ltrim( $name, '\\' );
    ...
    private function findServiceIdsContaining( Container $builder, string $name, bool $showHidden ): array {
