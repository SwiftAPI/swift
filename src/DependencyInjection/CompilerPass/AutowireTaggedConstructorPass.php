...
use Swift\DependencyInjection\LazyProxy\ProxyHelper;
...
class AutowireTaggedConstructorPass extends AbstractRecursivePass {
        ...
        foreach ( $parameters as $index => $parameter ) {
            ...
            if ( $checkAttributes ) {
                foreach ( $parameter->getAttributes() as $attribute ) {
                    if ( \Swift\DependencyInjection\Attributes\Autowire::class === $attribute->getName() ) {
                        $attribute = $attribute->newInstance();
                        if ( $attribute->getTag() ) {
                            $arguments[ $index ] = new TaggedIteratorArgument( $attribute->getTag() );
                        }
                        if ( $attribute->getServiceId() ) {
                            $value = $this->container->getParameterBag()->resolveValue( $attribute->getServiceId() );

                            $arguments[ $index ] = new Reference( $value, $parameter->allowsNull() ? ContainerInterface::NULL_ON_INVALID_REFERENCE : ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE );
                        }
                        break;
                    }
    ...
    private function createTypeAlternatives( ContainerBuilder $container, TypedReference $reference ): string {
        ...
        } else if ( isset( $this->ambiguousServiceTypes[ $type ] ) ) {
