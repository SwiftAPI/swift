...
/**
 * Class ScheduleListHelper
 * @package Honeywell\Helper
 */
class ScheduleListHelper {

    /**
     * ScheduleListHelper constructor.
     *
     * @param Configuration $configuration
     * @param array $schedule
     * @param array $addSchedules Schedules created during population
     */
    public function __construct(
        private Configuration $configuration,
        private array $schedule = array(),
        private array $addSchedules = array(),
    ) {
    }
    ...
    public function populate(array $defaultSchedules, array $recurringSchedules, array $onceSchedules, array $untilNextSchedules, string $from, string $to): void {
        ...
        $allSchedulesByID = array_merge($defaultSchedules, $recurringSchedules);
        $allSchedulesByID = array_merge($allSchedulesByID, $onceSchedules);
        $allSchedulesByID = array_merge($allSchedulesByID, $untilNextSchedules);
    ...
    private function appendSchedules(array $currentSchedules, array $schedulesToAppend, string $from, string $to): array {
        $timeZoneParam = $this->configuration->get('app.timezone', 'root') ?? 'Europe/Amsterdam';
        $timezone = new DateTimeZone($timeZoneParam);
        $fromDate = new DateTime($from, $timezone);
        $toDate   = new DateTime($to, $timezone);
    ...
    private function checkAndResolveConflict( stdClass $baseItem, stdClass $compareAgainst): array {
        ...
        if ($baseItem->type === ScheduleTypesEnum::RECURRING && $compareAgainst->type === ScheduleTypesEnum::DEFAULT) {
            $compareAgainstIsBoss = false;
        }
        if ($baseItem->type === ScheduleTypesEnum::ONCE && ($compareAgainst->type === ScheduleTypesEnum::DEFAULT || $compareAgainst->type === ScheduleTypesEnum::RECURRING)) {
            $compareAgainstIsBoss = false;
        }
        if ($baseItem->type === ScheduleTypesEnum::TILL_NEXT && ($compareAgainst->type === ScheduleTypesEnum::DEFAULT || $compareAgainst->type === ScheduleTypesEnum::RECURRING || $compareAgainst->type === ScheduleTypesEnum::ONCE)) {
            $compareAgainstIsBoss = false;
        }
    ...
    private function shouldScheduleByAppended( stdClass $scheduleToAppend, string $comparisonDate): bool {
        ...
        if ($scheduleToAppend->type === ScheduleTypesEnum::DEFAULT || $scheduleToAppend->type === ScheduleTypesEnum::RECURRING) {
        ...
        } elseif ($scheduleToAppend->type === ScheduleTypesEnum::ONCE || $scheduleToAppend->type === ScheduleTypesEnum::TILL_NEXT) {
    ...
    private function isScheduleActiveNow(string $dateToCheck, string $startTime, string $endTime): bool {
        if ( date('d-m-Y' ) !== date('d-m-Y', strtotime($dateToCheck))) {
            // It's not today
            return false;
        }
